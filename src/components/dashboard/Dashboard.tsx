...
const Dashboard: React.FC<DashboardProps> = ({ schools, reports }) => {
  ...
  const periods = Array.from(
    new Set(reports.map((report) => report.period))
  ).sort();

  // Prepare data for district distribution
  const districtData = Object.entries(stats.districtDistribution).map(
    ([name, value]) => ({
      name,
      value,
  ...
  const environmentData = [
    {
      ...
      percentage: (
        (stats.environmentDistribution.urban / stats.totalSchools) *
        100
      ).toFixed(1),
    },
    {
      ...
      percentage: (
        (stats.environmentDistribution.rural / stats.totalSchools) *
        100
      ).toFixed(1),
    },
  ];
  ...
  const prepareTrendData = () => {
    return periods
      .map((period) => {
        ...
        const avgTeacherUsage =
          periodReports.reduce(
            (sum, report) =>
              sum +
              (report.usage.teachers_using_ict / report.usage.total_teachers) *
                100,
            0
          ) / periodReports.length;

        const avgStudentLiteracy =
          periodReports.reduce(
            (sum, report) => sum + report.usage.student_digital_literacy_rate,
            0
          ) / periodReports.length;

        const avgFunctionalDevices =
          periodReports.reduce(
            (sum, report) => sum + report.infrastructure.functional_devices,
            0
          ) / periodReports.length;

        const internetAccessPercent =
          (periodReports.filter(
            (report) => report.infrastructure.internet_connection !== "None"
          ).length /
            periodReports.length) *
          100;

        const powerBackupPercent =
          (periodReports.filter((report) => report.infrastructure.power_backup)
            .length /
            periodReports.length) *
          100;
  ...
  const calculatePerformanceMetrics = () => {
    ...
    const latestReports = reports.filter(
      (report) => report.period === latestPeriod
    );
    ...
    const avgComputers =
      latestReports.reduce(
        (sum, report) => sum + report.infrastructure.computers,
        0
    ...
    const internetAccessCount = latestReports.filter(
      (report) => report.infrastructure.internet_connection !== "None"
    ).length;
    const powerBackupCount = latestReports.filter(
      (report) => report.infrastructure.power_backup
    ).length;
    ...
    const avgTeacherUsage =
      latestReports.reduce(
        (sum, report) =>
          sum +
          (report.usage.teachers_using_ict / report.usage.total_teachers) * 100,
        0
      ) / latestReports.length;
    const avgStudentLiteracy =
      latestReports.reduce(
    ...
    const avgWeeklyHours =
      latestReports.reduce(
        (sum, report) => sum + report.usage.weekly_computer_lab_hours,
        0
      ) / latestReports.length;

    // Capacity metrics
    const avgTrainedTeachers =
      latestReports.reduce(
        (sum, report) =>
          sum +
          (report.capacity.ict_trained_teachers / report.usage.total_teachers) *
            100,
        0
      ) / latestReports.length;
    const avgSupportStaff =
      latestReports.reduce(
        (sum, report) => sum + report.capacity.support_staff,
        0
      ) / latestReports.length;
    ...
    return {
      infrastructure: {
        ...
        internetAccessPercent: Math.round(
          (internetAccessCount / latestReports.length) * 100
        ),
        powerBackupPercent: Math.round(
          (powerBackupCount / latestReports.length) * 100
        ),
      },
      usage: {
  ...
  const prepareInfrastructureComparison = () => {
    ...
    return categories.map((category) => {
      ...
      ["Urban", "Rural"].forEach((environment) => {
        const environmentSchools = schools.filter(
          (school) => school.environment === environment.toLowerCase()
        );
        const environmentReports = environmentSchools
          .map((school) => getLatestReport(school.id, reports))
          .filter(Boolean);
        ...
        if (environmentReports.length > 0) {
          ...
          environmentReports.forEach((report) => {
            if (report) {
              switch (category) {
                case "Computers":
                  total += report.infrastructure.computers;
                  break;
                case "Tablets":
                  total += report.infrastructure.tablets;
                  break;
                case "Projectors":
                  total += report.infrastructure.projectors;
                  break;
                case "Printers":
                  total += report.infrastructure.printers;
                  break;
              }
            }
          });
          dataPoint[environment] = Math.round(
            total / environmentReports.length
          );
        } else {
  ...
  const calculateAlerts = () => {
    ...
    const noInternetSchools = latestReports.filter(
      (report) => report.infrastructure.internet_connection === "None"
    ).length;
    ...
    const lowUsageSchools = latestReports.filter(
      (report) =>
    ...
    const lowDeviceSchools = latestReports.filter(
      (report) => report.infrastructure.functional_devices < 10
    ).length;
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <h3 className="text-2xl font-bold">
                {performanceMetrics?.infrastructure.internetAccessPercent ||
                  stats.schoolsWithInternetPercent.toFixed(0)}
                %
              </h3>
              <p className="text-amber-200 text-xs">
                {performanceMetrics
                  ? `${performanceMetrics.period} data`
                  : "Overall average"}
              </p>
            </div>
          </div>
        ...
        <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-green-200 text-xs">Average across schools</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-purple-200 text-xs">Digital literacy rate</p>
            </div>
          </div>
        </Card>
      </div>
          ...
          {performanceMetrics ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <div className="mt-2 space-y-1">
                    <div className="text-xs text-blue-600">
                      Avg Computers:{" "}
                      {performanceMetrics.infrastructure.avgComputers}
                    </div>
                    <div className="text-xs text-blue-600">
                      Functional Devices:{" "}
                      {performanceMetrics.infrastructure.avgFunctionalDevices}
                    </div>
                    <div className="text-xs text-blue-600">
                      Power Backup:{" "}
                      {performanceMetrics.infrastructure.powerBackupPercent}%
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <div className="text-xs text-green-600">
                      Student Literacy:{" "}
                      {performanceMetrics.usage.avgStudentLiteracy}%
                    </div>
                    <div className="text-xs text-green-600">
                ...
                <div className="bg-amber-50 p-4 rounded-lg">
                  ...
                  <div className="mt-2 space-y-1">
                    <div className="text-xs text-amber-600">
                      Trained Teachers:{" "}
                      {performanceMetrics.capacity.avgTrainedTeachers}%
                    </div>
                    <div className="text-xs text-amber-600">
                      Support Staff:{" "}
                      {performanceMetrics.capacity.avgSupportStaff}
                    </div>
                  </div>
                </div>
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-2">
                Add periodic observations to see performance metrics
              </p>
            </div>
          )}
        </Card>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {schools.slice(0, 5).map((school, index) => {
                ...
                const schoolReports = reports.filter(
                  (r) => r.school_id === school.id
                );
                const { level, score } =
                  calculateICTReadinessLevel(schoolReports);

                const strengths = [];
                if (latestReport) {
                  if (latestReport.infrastructure.functional_devices >= 20)
                    strengths.push("Good Infrastructure");
                  if (
                    latestReport.infrastructure.internet_connection !== "None"
                  )
                    strengths.push("Internet Access");
                  if (
                    latestReport.usage.teachers_using_ict /
                      latestReport.usage.total_teachers >=
                    0.7
                  )
                    strengths.push("High Teacher Usage");
                  if (latestReport.usage.student_digital_literacy_rate >= 70)
                    strengths.push("High Student Literacy");
                }
