...
const SchoolForm: React.FC<SchoolFormProps> = ({ school, onSubmit, onCancel }) => {
  ...
  useEffect(() => {
    const totalClassrooms = formData.permanentClassrooms + formData.semiPermanentClassrooms + formData.temporaryClassrooms;
    if (totalClassrooms > 0 && formData.totalStudents > 0) {
      const ratio = Math.round((formData.totalStudents / totalClassrooms) * 10) / 10;
      if (ratio !== formData.pupilClassroomRatio) {
        setFormData(prev => ({ ...prev, pupilClassroomRatio: ratio }));
      }
    }
  }, [formData.totalStudents, formData.permanentClassrooms, formData.semiPermanentClassrooms, formData.temporaryClassrooms]);
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Basic Information */}
        {renderSection('basic', 'Basic Information', SchoolIcon, (
          <>
            {renderInput('School Name', 'name', 'text', undefined, true, 'Enter school name')}
            {renderInput('Region', 'region', 'select', [
            ...
            {renderInput('District', 'district', 'text', undefined, true, 'Enter district')}
            {renderInput('Sub-County', 'subCounty', 'text', undefined, true, 'Enter sub-county')}
            {renderInput('EMIS Number', 'emisNumber', 'text', undefined, false, 'Enter EMIS number')}
            {renderInput('UPI Code', 'upiCode', 'text', undefined, false, 'Enter UPI code')}
            {renderInput('School Type', 'type', 'select', [
              { value: 'Public', label: 'Public' },
              { value: 'Private', label: 'Private' }
            ], true)}
            {renderInput('Environment', 'environment', 'select', [
            ...
            {renderInput('Year Established', 'yearEstablished', 'number', undefined, false, undefined, 1900, new Date().getFullYear())}
            {renderInput('Signature Program', 'signatureProgram', 'text', undefined, false, 'Special programs or focus areas')}
            {renderInput('Latitude', 'latitude', 'number', undefined, true, undefined, -90, 90, 0.000001)}
            {renderInput('Longitude', 'longitude', 'number', undefined, true, undefined, -180, 180, 0.000001)}
            {renderInput('Male Students', 'maleStudents', 'number', undefined, true, undefined, 0)}
            {renderInput('Female Students', 'femaleStudents', 'number', undefined, true, undefined, 0)}
            {renderInput('Total Students (Auto Calculated)', 'totalStudents', 'number', undefined, false, 'Auto-calculated', 0, undefined, undefined, true)}
            {renderInput('Head Teacher', 'headTeacher', 'text', undefined, true, 'Enter head teacher name')}
            {renderInput('Email', 'email', 'email', undefined, true, 'Enter school email')}
            {renderInput('Phone', 'phone', 'tel', undefined, true, 'Enter school phone number')}
          </>
        ))}

        {/* ICT Infrastructure */}
        {renderSection('infrastructure', 'ICT Infrastructure', Monitor, (
          <>
            {renderInput('Student Computers', 'studentComputers', 'number', undefined, false, undefined, 0)}
            {renderInput('Teacher Computers', 'teacherComputers', 'number', undefined, false, undefined, 0)}
            {renderInput('Multimedia Projectors', 'projectors', 'number', undefined, false, undefined, 0)}
            {renderInput('Smart Boards', 'smartBoards', 'number', undefined, false, undefined, 0)}
            {renderInput('Tablets', 'tablets', 'number', undefined, false, undefined, 0)}
            {renderInput('Laptops', 'laptops', 'number', undefined, false, undefined, 0)}
            {renderInput('Has Computer Laboratory', 'hasComputerLab', 'checkbox')}
            ...
            {renderInput('Has Secure Storage Room', 'hasSecureRoom', 'checkbox')}
            {renderInput('Has Appropriate Furniture', 'hasFurniture', 'checkbox')}
            {renderCheckboxGroup('Power Backup Solutions', 'powerBackup', ['Solar', 'Generator', 'UPS'])}
          </>
        ))}

        {/* Internet Connectivity */}
        {renderSection('connectivity', 'Internet Connectivity', Wifi, (
          <>
            ...
            {renderInput('Bandwidth (Mbps)', 'bandwidthMbps', 'number', undefined, false, undefined, 0, undefined, 0.1)}
            {renderInput('Connection Stability', 'stability', 'select', [
              { value: 'High', label: 'High' },
              { value: 'Medium', label: 'Medium' },
              { value: 'Low', label: 'Low' }
            ])}
            {renderInput('Internet Service Provider', 'provider', 'text', undefined, false, 'Enter ISP name')}
            {renderInput('Connection is Stable', 'isStable', 'checkbox')}
            {renderInput('Has Internet Usage Policy', 'hasUsagePolicy', 'checkbox')}
            {renderCheckboxGroup('Wi-Fi Coverage Areas', 'wifiCoverage', [
              'Administration', 'Classrooms', 'Library', 'Dormitories', 'Staff Room', 'Computer Lab'
            ])}
          </>
        ))}

        {/* ICT Software and Digital Resources */}
        {renderSection('software', 'ICT Software and Digital Resources', Settings, (
          <>
            {renderInput('Has Learning Management System', 'hasLMS', 'checkbox')}
            {renderInput('LMS Name', 'lmsName', 'text', undefined, false, 'e.g., Moodle, Google Classroom')}
            {renderInput('Has Licensed Educational Software', 'hasLicensedSoftware', 'checkbox')}
            {renderInput('Has Productivity Software Suite', 'hasProductivitySuite', 'checkbox')}
            {renderInput('Has Digital Library', 'hasDigitalLibrary', 'checkbox')}
            {renderInput('Has Local Digital Content', 'hasLocalContent', 'checkbox')}
            {renderInput('Content Source', 'contentSource', 'textarea', undefined, false, 'Describe sources of digital content')}
            {renderCheckboxGroup('Licensed Software', 'licensedSoftware', [
              'Microsoft Office 365', 'Adobe Creative Cloud', 'Educational Games Suite', 
              'Language Lab Software', 'STEM Learning Suite', 'Typing Tutor', 'Digital Science Lab'
            ])}
            {renderCheckboxGroup('Productivity Suites', 'productivitySuite', [
              'Microsoft Office', 'Google Workspace', 'LibreOffice', 'Other'
            ])}
          </>
        ))}

        {/* Human Capacity and ICT Competency */}
        {renderSection('humanCapacity', 'Human Capacity and ICT Competency', Users, (
          <>
            {renderInput('ICT-Trained Teachers', 'ictTrainedTeachers', 'number', undefined, false, undefined, 0)}
            {renderInput('Male Teachers', 'maleTeachers', 'number', undefined, false, undefined, 0)}
            {renderInput('Female Teachers', 'femaleTeachers', 'number', undefined, false, undefined, 0)}
            {renderInput('Total Teachers', 'totalTeachers', 'number', undefined, false, 'Auto-calculated', 0)}
            {renderInput('P5-P7 Teachers', 'p5ToP7Teachers', 'number', undefined, false, undefined, 0)}
            {renderInput('ICT Support Staff', 'supportStaff', 'number', undefined, false, undefined, 0)}
            {renderInput('Monthly ICT Training Sessions', 'monthlyTrainings', 'number', undefined, false, undefined, 0)}
            {renderInput('Teacher ICT Competency Level', 'teacherCompetencyLevel', 'select', [
              { value: 'Basic', label: 'Basic' },
              { value: 'Intermediate', label: 'Intermediate' },
              { value: 'Advanced', label: 'Advanced' }
            ])}
            {renderInput('Has Ongoing Capacity Building', 'hasCapacityBuilding', 'checkbox')}
          </>
        ))}

        {/* Pedagogical ICT Usage */}
        {renderSection('pedagogical', 'Pedagogical ICT Usage', BookOpen, (
          <>
            {renderInput('ICT-Integrated Lessons per Week', 'ictIntegratedLessons', 'number', undefined, false, undefined, 0)}
            {renderInput('Uses ICT for Assessments', 'usesICTAssessments', 'checkbox')}
            {renderInput('Students Create ICT Projects', 'hasStudentProjects', 'checkbox')}
            {renderInput('Uses Blended Learning', 'usesBlendedLearning', 'checkbox')}
            {renderInput('Has Assistive Technology', 'hasAssistiveTech', 'checkbox')}
            {renderInput('Digital Tool Usage Frequency', 'digitalToolUsageFrequency', 'select', [
              { value: 'Daily', label: 'Daily' },
              { value: 'Weekly', label: 'Weekly' },
              { value: 'Rarely', label: 'Rarely' },
              { value: 'Never', label: 'Never' }
            ])}
            {renderInput('Has Digital Learning Content', 'hasDigitalContent', 'checkbox')}
            {renderInput('Has Teacher Peer Support', 'hasPeerSupport', 'checkbox')}
          </>
        ))}

        {/* ICT Governance and Policy */}
        {renderSection('governance', 'ICT Governance and Policy', Shield, (
          <>
            {renderInput('Has School ICT Policy', 'hasICTPolicy', 'checkbox')}
            {renderInput('Aligned with National ICT Strategy', 'alignedWithNationalStrategy', 'checkbox')}
            {renderInput('Has ICT Committee/Coordinator', 'hasICTCommittee', 'checkbox')}
            {renderInput('Has ICT Budget Allocation', 'hasICTBudget', 'checkbox')}
            {renderInput('Has ICT Monitoring System', 'hasMonitoringSystem', 'checkbox')}
            {renderInput('Has Active School Management Committee', 'hasActiveSMC', 'checkbox')}
            {renderInput('Has Active Parent-Teacher Association', 'hasActivePTA', 'checkbox')}
            {renderInput('Has Local Leader Engagement', 'hasLocalLeaderEngagement', 'checkbox')}
          </>
        ))}

        {/* Students' Digital Literacy and Engagement */}
        {renderSection('studentEngagement', 'Students\' Digital Literacy and Engagement', GraduationCap, (
          <>
            {renderInput('Student Digital Literacy Level', 'digitalLiteracyLevel', 'select', [
              { value: 'Basic', label: 'Basic' },
              { value: 'Intermediate', label: 'Intermediate' },
              { value: 'Advanced', label: 'Advanced' }
            ])}
            {renderInput('Has ICT Club', 'hasICTClub', 'checkbox')}
            {renderInput('Students Use Online Platforms', 'usesOnlinePlatforms', 'checkbox')}
            {renderInput('Student Feedback Rating (1-5)', 'studentFeedbackRating', 'number', undefined, false, undefined, 1, 5)}
            {renderInput('Students Using Digital Content', 'studentsUsingDigitalContent', 'number', undefined, false, undefined, 0)}
          </>
        ))}

        {/* Community and Parental Engagement */}
        {renderSection('community', 'Community and Parental Engagement', Globe, (
          <>
            {renderInput('Has Parent Digital Portal', 'hasParentPortal', 'checkbox')}
            {renderInput('Has Community ICT Outreach', 'hasCommunityOutreach', 'checkbox')}
            {renderInput('Has Industry Partnerships', 'hasIndustryPartners', 'checkbox')}
            {renderInput('Partner Organizations', 'partnerOrganizations', 'textarea', undefined, false, 'List partner organizations (one per line)')}
            {renderInput('NGO Support', 'ngoSupport', 'textarea', undefined, false, 'List supporting NGOs (one per line)')}
            {renderInput('Community Contributions', 'communityContributions', 'textarea', undefined, false, 'Describe community contributions')}
          </>
        ))}
        ...
        {/* Accessibility */}
        {renderSection('accessibility', 'Accessibility', MapPin, (
          <>
            {renderInput('Distance from HQ (km)', 'distanceFromHQ', 'number', undefined, false, undefined, 0, undefined, 0.1)}
            {renderInput('Accessible All Year', 'isAccessibleAllYear', 'checkbox')}
            {renderInput('Is Inclusive School', 'isInclusive', 'checkbox')}
            {renderInput('Serves Girls', 'servesGirls', 'checkbox')}
            {renderInput('Serves Persons with Disabilities', 'servesPWDs', 'checkbox')}
            {renderInput('Serves Refugees', 'servesRefugees', 'checkbox')}
            {renderInput('Only School in Area', 'isOnlySchoolInArea', 'checkbox')}
          </>
        ))}

        {/* School Facilities & Environment */}
        {renderSection('facilities', 'School Facilities & Environment', Building, (
          <>
            {renderInput('Permanent Classrooms', 'permanentClassrooms', 'number', undefined, false, undefined, 0)}
            {renderInput('Semi-Permanent Classrooms', 'semiPermanentClassrooms', 'number', undefined, false, undefined, 0)}
            {renderInput('Temporary Classrooms', 'temporaryClassrooms', 'number', undefined, false, undefined, 0)}
            {renderInput('Pupil-Classroom Ratio', 'pupilClassroomRatio', 'number', undefined, false, 'Auto-calculated', 0, undefined, 0.1)}
            {renderInput('Boys Toilets', 'boysToilets', 'number', undefined, false, undefined, 0)}
            {renderInput('Girls Toilets', 'girlsToilets', 'number', undefined, false, undefined, 0)}
            {renderInput('Staff Toilets', 'staffToilets', 'number', undefined, false, undefined, 0)}
            ...
            {renderInput('School Accessibility', 'schoolAccessibility', 'select', [
              { value: 'All-Weather', label: 'All-Weather Road' },
              { value: 'Seasonal', label: 'Seasonal Access' },
              { value: 'Remote', label: 'Remote/Difficult Access' }
            ])}
            {renderInput('Nearby Health Facility', 'nearbyHealthFacility', 'text', undefined, false, 'Name of nearest health facility')}
            {renderInput('Health Facility Distance (km)', 'healthFacilityDistance', 'number', undefined, false, undefined, 0, undefined, 0.1)}
            {renderCheckboxGroup('Security Infrastructure', 'securityInfrastructure', [
              'Perimeter Wall', 'Fence', 'Security Guard', 'CCTV', 'Lighting', 'None'
            ])}
          </>
        ))}

        {/* Performance */}
        {renderSection('performance', 'Performance', TrendingUp, (
          <>
            {renderInput('PLE Pass Rate Year 1 (%)', 'plePassRateYear1', 'number', undefined, false, undefined, 0, 100, 0.1)}
            {renderInput('PLE Pass Rate Year 2 (%)', 'plePassRateYear2', 'number', undefined, false, undefined, 0, 100, 0.1)}
            {renderInput('PLE Pass Rate Year 3 (%)', 'plePassRateYear3', 'number', undefined, false, undefined, 0, 100, 0.1)}
            {renderInput('Literacy Trends', 'literacyTrends', 'textarea', undefined, false, 'Describe literacy performance trends')}
            {renderInput('Numeracy Trends', 'numeracyTrends', 'textarea', undefined, false, 'Describe numeracy performance trends')}
            {renderInput('Innovations', 'innovations', 'textarea', undefined, false, 'Describe school innovations and initiatives')}
            {renderInput('Unique Achievements', 'uniqueAchievements', 'textarea', undefined, false, 'Describe unique achievements and awards')}
          </>
        ))}
